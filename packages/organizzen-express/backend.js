...
connectToMongoDB()
    .then((client) => {
        ...
        app.get('/events/:eventId/tasks', async (req, res) => {
            ...
            try {
                const eventTasks = await tasksCollection.find({ eventId }).toArray();
                res.json(eventTasks);
            } catch (error) {
                console.error('Error fetching tasks:', error);
                res.status(500).json({ error: 'Failed to fetch tasks' });
        ...
        app.get('/events/:eventId/tasks/:taskId/mark-as-done', async (req, res) => {
            const eventId = req.params.eventId;
            const taskId = req.params.taskId;
        ...
        app.put('/events/:eventId/tasks/:taskId/undo', async (req, res) => {
            ...
            try {
                ...
                res.status(500).json({ error: 'Failed to undo mark task as done' });
            }
        });
        ...
        const deleteTask = async (eventId, taskId) => {
            try {
                const result = await tasksCollection.deleteOne({ eventId, taskId });
